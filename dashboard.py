...
with st.sidebar:
    ...
    steps = [
        ("1", "🔗 Paste YouTube URL <em>(optional for unpublished)</em>"),
        ("2", "🎬 Upload your video as <code>.mp4</code>"),
...
with col1:
    yt_input = st.text_input(
        "🔗 YouTube Video URL (optional for unpublished videos)",
...
if is_predict:
    ...
    <div class="glass-card" style="border-left: 4px solid #8b5cf6; padding: 1rem 1.5rem; margin-bottom:1rem;">
        <span class="predict-badge">🔮 Pre-Publish Prediction Mode</span>
        <div style="color:var(--text-secondary); font-size:0.9rem; margin-top:0.3rem;">
            No YouTube data available yet — ad spots are predicted using <strong>audio energy analysis</strong>.
...
# ═══════════════════════════════════════════════════════════════
st.markdown(
    f'<div class="section-header">{"🔮 Predicted" if is_predict else "🎯"} Where to Place Your Ads</div>',
    unsafe_allow_html=True
)
...
if not recs:
    ...
    for idx, r in enumerate(recs):
        ...
        html = (
            ...
            f'<div>'
            f'<span class="placement-title">📍 Ad Spot {r["placement_number"]}</span>'
            f'&nbsp;&nbsp;'
...
if cands_list:
    ...
    if is_predict:
        tips.append(("🔮", "This was a <strong>predicted</strong> analysis. Once published, re-analyze with your YouTube URL for real retention data and more accurate spots."))
